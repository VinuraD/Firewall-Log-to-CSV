...
func createcsvdat(rows map[string]int, keys []string) {
	...
	for _, key := range keys {
		if len(key) > 0 {
			w.Write(strings.Split(key+" "+strconv.Itoa(rows[key]), " "))
		} else {
		}
	}
...
func main() {
	...
	fmt.Println("Filtered from original log file, ", len(olines))
	...
	for _, row := range olines {
		counter[row]++
	}
	fmt.Println("Unique records found, ", len(counter))
	//fmt.Println(counter)
